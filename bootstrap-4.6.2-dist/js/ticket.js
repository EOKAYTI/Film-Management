...
function getCurrentDate(){
  ...
  var month = (currentDate.getMonth() + 1).toString().padStart(2, '0'); // Ghi chú: Tháng trong JavaScript bắt đầu từ 0 (tháng 0 là tháng 1), vì vậy chúng ta cộng thêm 1 vào kết quả.
    ...
    //    console.log(response);
      ...
      if(authenticationReturn!=null){
        if(authenticationReturn.status ==true){
          if(authenticationReturn.data != 2){
...
function generateNameFilmOpt(){
  $.ajax({
    ...
    <soapenv:Body>
      <q0:getByComplexData>
        <q0:data>${getCurrentDate()}</q0:data>
      </q0:getByComplexData>
    </soapenv:Body>
  </soapenv:Envelope>`,
    contentType: "text/xml",
    ...
    statusCode: {
      ...
      200: function() {
          // Xử lý lỗi 401 (Unauthorized)
          console.log("Lỗi 200 - Không được phép truy cập");
          // signined = false;
          messageReturn = "Vui lòng đăng nhập để tiếp tục!";
      }
  },
      ...
      console.log(docsReturn);
        ...
        if(docsReturn!=null){
            ...
            docsReturn.data.forEach(function(element){
              tbody+=` 
              <option value="${element.id}">${element.name} ${element.time.time}</option>
             `;
            });
            
             $("#film-name").html(tbody);
...
function updatePrice(id){
  ...
  $.ajax({
    ...
    success: function(response) {
    //    console.log(response);
      docs= $(response).find("findPriceByIdReturn").text();
      console.log("docs:",docs);
  ...
  function generateSeatFilmOpt(filmId){
    $.ajax({
      url: "http://localhost:8080/MovieTicketManagementVer3/services/SeatServiceImp",
      type: "POST",
      dataType: "xml",
      data: `<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:q0="http://imp.service" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
      <soapenv:Body>
        <q0:findAvailableByFirmId>
          <q0:id>${filmId}</q0:id>
        </q0:findAvailableByFirmId>
      </soapenv:Body>
    </soapenv:Envelope>`,
      ...
      success: function(response) {
      //    console.log(response);
        docs= $(response).find("findAvailableByFirmIdReturn").text();
        console.log("docs:",docs);
        ...
        console.log(docsReturn);
          ...
          if(docsReturn!=null){
              ...
              docsReturn.data.forEach(function(element){
                tbody+=` 
                <option value="${element.id}">Ghế ${element.seatNumber}</option>
               `;
              });
...
function generateData(){
  ...
  $.ajax({
    url: "http://localhost:8080/MovieTicketManagementVer3/services/OrderServiceImp",
    type: "POST",
    dataType: "xml",
    data: `<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:q0="http://imp.service" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <soapenv:Body>
      ...
      console.log(docsReturn);
        ...
        if(docsReturn!=null){
            ...
            docsReturn.data.forEach(function(element){
              ...
              <td>${element.firm.time.time}</td>
              <td>${element.firm.price}</td>
              <td> Ghế số ${element.seat.seatNumber}</td>
...
$(document).ready(function () {
    ...
    $('#btn_save').click(function (e) {
        let firmId = $('#film-name').val();
        let customerName = $('#customer-name').val();
        let seatId = $('#seat').val();
        console.log("check 1:",firmId);
        console.log("check 2:",customerName);
        console.log("check 3:",seatId);
        // validating
        let isValid = false;
        let message = "";
        if(firmId==null||firmId==""){
         ...
         //  console.log("not valid")
          ...
          message ="Vui lòng chọn vị trí ghế.";
         //  console.log("not valid")
        }
        ...
        if(isValid){
            ...
            $.ajax({
              ...
              <soapenv:Body>
                <q0:create>
                  <q0:model>
                    <q1:customerName>${customerName}</q1:customerName>
                    <q1:email>none</q1:email>
                    <q1:firm>
                      <q1:category>
                        <q1:id>0</q1:id>
                        <q1:name>0</q1:name>
                    ...
                    <q1:phoneNumber>0</q1:phoneNumber>
                    <q1:seat>
                      <q1:firm>
                        <q1:category>
                          <q1:id>0</q1:id>
